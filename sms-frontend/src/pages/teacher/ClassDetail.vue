<template>
  <div class="space-y-6">
    <div class="bg-gradient-to-r from-brand-red to-brand-orange p-6 md:p-8 rounded-3xl text-white shadow-md relative overflow-hidden">
      <svg class="absolute top-0 right-0 transform translate-x-1/4 -translate-y-1/4 opacity-10 w-64 h-64" fill="currentColor" viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
      
      <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <button @click="goBack" class="mb-4 flex items-center text-white/80 hover:text-white transition-colors text-sm font-semibold">
          ...
          <h1 class="text-3xl md:text-4xl font-bold font-serif mt-3">Kelas {{ classInfo.name }}</h1>
          <p class="text-white/90 text-sm mt-1">Tahun Ajaran 2023/2024 • Semester Ganjil</p>
        </div>
        <div class="bg-white/10 p-4 rounded-2xl backdrop-blur-sm border border-white/20 text-center min-w-[120px]">
          ...
          <p class="text-xs text-white/80 font-medium uppercase tracking-wider mt-1">Total Siswa</p>
        </div>
      </div>
    </div>

    <div class="bg-white p-2 rounded-2xl shadow-sm border border-gray-200 inline-flex w-full md:w-auto overflow-x-auto">
      <button 
        v-for="tab in tabs" 
        :key="tab.id"
        @click="activeTab = tab.id"
        class="px-6 py-2.5 rounded-xl text-sm font-semibold transition-all whitespace-nowrap"
        :class="activeTab === tab.id ? 'bg-brand-red text-white shadow-sm' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'"
      >
        {{ tab.label }}
      </button>
    </div>

    <div class="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
      ...
      >
        <template #cell(name)="{ item }">
          <div class="flex items-center h-full font-medium text-gray-800">
            <router-link
              :to="{ name: 'TeacherStudentProfile', params: { id: item.id } }"
        ...
        <template #cell(gender)="{ item }">
          <span :class="item.gender === 'L' ? 'text-blue-600 bg-blue-50' : 'text-pink-600 bg-pink-50'" class="px-2.5 py-1 rounded-md text-xs font-bold">
        ...
        <template #cell(attendance_rate)="{ item }">
          <div class="flex items-center gap-2">
            <div class="w-full bg-gray-200 rounded-full h-2 max-w-[100px]">
              <div class="bg-green-500 h-2 rounded-full" :style="`width: ${item.attendance_rate}%`"></div>
            </div>
    ...
    >
      <div class="p-2 space-y-4">
        <div class="flex items-center gap-4 border-b border-gray-100 pb-4">
          <div class="w-16 h-16 rounded-full bg-brand-red/10 flex items-center justify-center text-brand-red font-bold text-xl">
            {{ selectedStudent?.name?.charAt(0) }}
          </div>
          <div>
            <p class="font-bold text-lg text-gray-800">{{ selectedStudent?.name }}</p>
            <p class="text-sm text-gray-500">NIS/NISN: {{ selectedStudent?.nis }}</p>
          </div>
        </div>
        <p class="text-sm text-gray-600">Fitur detail siswa secara mendalam (Grafik nilai individu, riwayat pelanggaran, dll) dapat ditambahkan di sini.</p>
        <div class="pt-4 text-right">
          <button @click="isModalOpen = false" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-200 transition-colors">
            Tutup
          </button>
        </div>
      </div>
...
const fetchClassDetail = async (dateFilter = null) => {
  ...
  try {
    ...
    toastStore.error(error.response?.data?.error || "Gagal memuat detail kelas perwalian.");
  } finally {
