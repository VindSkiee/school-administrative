<template>
  <div class="space-y-6">
    <div class="bg-gradient-to-r from-brand-red to-brand-orange rounded-3xl p-6 md:p-8 text-white shadow-md">
      <h1 class="text-2xl md:text-3xl font-bold font-serif">Buku Nilai (Gradebook)</h1>
      <p class="text-orange-100 text-sm mt-1 max-w-xl font-medium">
        Kelola dan input nilai siswa secara langsung. Nilai akan tersimpan otomatis saat Anda mengubah sel.
      </p>
    </div>
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4 items-end">
      ...
      <div class="flex-1 w-full md:w-auto">
        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">
          {{ isHomeroomMode ? 'Mata Pelajaran (Filter)' : 'Kelas & Mata Pelajaran' }}
        </label>
        <BaseSelect
          v-if="!isHomeroomMode"
      ...
      <div class="flex items-center gap-2 shrink-0">
        <span v-if="gradingWeights && !isHomeroomMode && selectedScheduleId" class="text-[10px] font-bold text-gray-400 bg-gray-50 px-3 py-2 rounded-lg border border-gray-100 whitespace-nowrap">
          Bobot: Tugas {{ gradingWeights.task }}% | UTS {{ gradingWeights.uts }}% | UAS {{ gradingWeights.uas }}% | Kehadiran {{ gradingWeights.attendance }}%
        </span>
        <button
          v-if="hasHomeroomClass"
          @click="toggleHomeroomMode"
          class="flex items-center gap-2 px-4 py-2.5 text-xs font-bold rounded-lg border transition-colors whitespace-nowrap"
          :class="isHomeroomMode
    ...
    <div v-if="selectedAcademicYearId && !isActiveYear && selectedScheduleId && !isHomeroomMode" class="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
      <svg class="w-5 h-5 text-amber-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
      <div>
        <p class="text-sm font-bold text-amber-800">Mode Baca Saja (Read-Only)</p>
        <p class="text-xs text-amber-600">Tahun ajaran ini tidak aktif. Pengeditan nilai hanya tersedia pada tahun ajaran aktif.</p>
      </div>
    </div>

    <div v-if="isLoading" class="flex flex-col items-center justify-center py-16">
      <div class="animate-spin rounded-full h-10 w-10 border-4 border-gray-100 border-t-brand-red mb-3"></div>
      <p class="text-gray-500 font-medium text-sm">Memuat data gradebook...</p>
    </div>

    <!-- ==================== HOMEROOM MODE ==================== -->
    <template v-else-if="isHomeroomMode">
      <div class="bg-white border border-blue-200 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-lg font-bold text-gray-800">Rekap Kelas Wali — {{ homeroomClassDisplayName }}</h3>
            <p class="text-xs text-gray-500">Nilai akhir per mata pelajaran. Mode ini hanya baca. Gunakan filter di atas untuk fokus pada mapel tertentu.</p>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <span class="px-2.5 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded-lg">
            {{ homeroomRowData.length }} Siswa
          </span>
          <span class="px-2.5 py-1 bg-blue-100 text-blue-700 text-[10px] font-bold rounded-lg">
            {{ homeroomAllSubjects.length }} Mapel
          </span>
        </div>
      </div>

      <div v-if="isLoadingRecap" class="flex flex-col items-center justify-center py-16">
        <div class="animate-spin rounded-full h-10 w-10 border-4 border-gray-100 border-t-blue-600 mb-3"></div>
        <p class="text-gray-500 font-medium text-sm">Memuat rekap wali kelas...</p>
      </div>
    ...
    <!-- ==================== EMPTY STATE ==================== -->
    <div v-else-if="!selectedScheduleId" class="bg-white rounded-3xl p-12 text-center shadow-sm border border-gray-200">
      ...
      <h3 class="text-lg font-bold text-gray-800">Pilih Tahun Ajaran dan Kelas</h3>
      <p class="text-gray-500 mt-1 text-sm max-w-md mx-auto">
        Silakan pilih tahun ajaran dan kelas/mata pelajaran di atas untuk mulai melihat dan menginput nilai siswa.
      </p>
    </div>
    ...
    <div v-if="savingIndicator" class="fixed bottom-6 right-6 z-50 bg-green-600 text-white px-4 py-2.5 rounded-xl shadow-lg flex items-center gap-2 text-sm font-bold animate-fade-in-up">
...
const extractClassName = (s) => {
  if (!s) return '';
  return s.class_name || s.class?.name || s.schoolClass?.name || s.school_class?.name || '';
};
...
const selectedSchedule = computed(() => {
  return teacherSchedules.value.find(s => s.id === selectedScheduleId.value) || null;
});
...
const selectedYear = computed(() => {
  return academicYears.value.find(ay => ay.id === selectedAcademicYearId.value);
});
...
const filteredHomeroomSubjects = computed(() => {
  ...
  return homeroomAllSubjects.value.filter(s => s.id === selectedSubjectFilter.value);
});
...
const typeColor = (type) => {
  switch (type) {
    case 'uts': return 'bg-brand-orange/10 text-brand-orange';
    case 'uas': return 'bg-brand-red/10 text-brand-red';
...
const columnDefs = computed(() => {
  ...
  cols.push({
    ...
    cellStyle: (params) => {
      ...
      if (val >= 90) return { textAlign: 'center', color: '#16a34a', fontWeight: 'bold' };
      if (val >= 75) return { textAlign: 'center', color: '#d97706', fontWeight: 'bold' };
      return { textAlign: 'center', color: '#E02E2B', fontWeight: 'bold' };
    },
    valueFormatter: (params) => params.value != null ? `${params.value}%` : '-',
  });

  const editable = isActiveYear.value;
  ...
  cols.push({
    ...
    pinned: 'right',
    width: 150,
    cellStyle: { fontWeight: 'bold', backgroundColor: '#f9fafb', textAlign: 'center' },
...
const homeroomRowData = computed(() => homeroomRecapData.value.map(s => ({ ...s })));

// ==================== WEIGHTED AVERAGE CALCULATION ====================
const calculateWeightedAverage = (studentData) => {
  if (!studentData?.assignments) return null;

  const weights = gradingWeights.value || { task: 40, uts: 25, uas: 25, attendance: 10 };
  ...
  if (taskAvg !== null) { weightedSum += taskAvg * weights.task; activeWeight += weights.task; }
  if (utsAvg !== null) { weightedSum += utsAvg * weights.uts; activeWeight += weights.uts; }
  if (uasAvg !== null) { weightedSum += uasAvg * weights.uas; activeWeight += weights.uas; }
...
const onCellValueChanged = (params) => {
  ...
  const newScore = rawScore != null && rawScore !== '' ? Number(rawScore) : null;

  // Instant save — no debounce
  saveScoreSilently(studentId, assignmentId, newScore);
...
const loadGradebook = async () => {
  ...
  try {
    ...
    gradingWeights.value = payload.weights || { task: 40, uts: 25, uas: 25, attendance: 10 };
  } catch (error) {
