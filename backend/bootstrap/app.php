...
return Application::configure(basePath: dirname(__DIR__))
    ->withRouting(
        ...
        then: function () {
            ...
            Route::middleware(['api', 'auth:api', 'password.changed', 'role:admin'])
                ->prefix('api/v1/admin')
                ->group(base_path('routes/api/v1/admin.php'));

            // 3. Route Teacher (Protected by Role: Teacher)
            Route::middleware(['api', 'auth:api', 'password.changed', 'role:teacher'])
                ->prefix('api/v1/teacher')
                ->group(base_path('routes/api/v1/teacher.php'));

            // 4. Route Student (Protected by Role: Student)
            Route::middleware(['api', 'auth:api', 'password.changed', 'role:student'])
                ->prefix('api/v1/student')
                ->group(base_path('routes/api/v1/student.php'));

            Route::middleware(['api', 'auth:api', 'password.changed', 'role:principal'])
